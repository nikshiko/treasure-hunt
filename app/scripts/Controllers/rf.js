...
myApp.controller('rfController',
    function rfController($scope,$http,$window,$uibModalInstance,DataService){
        ...
        $scope.submitForm = function(){
            ...
            $http.post('http://52.208.32.211:8082/reg/check', data)
            .success(function(data, status, headers, config) {
                ...
                $http.post('http://52.208.32.211:8082/reg/put', data)

                    .success(function(data, status, headers, config) {
                        if(data == "exists")
                        $window.location.href = "http://52.208.32.211:8000/#/failure";
                        else if(data == "registered")
                            $window.location.href = "http://52.208.32.211:8000/#/success";
...
.controller('loginController',
    function loginController($scope,$http,$window,$uibModalInstance,userPersistenceService){
        ...
        $scope.submitForm = function(){
            ...
            $http.post('http://52.208.32.211:8082/login', data)
                .success(function(data, status, headers, config) {
                    userPersistenceService.setCookieData($scope.editableEmployee.tpxid);

                    if(data == "0")
                    $window.location.href = "http://52.208.32.211:8000/#/Level1";
                    else
                    if(data == "1")
